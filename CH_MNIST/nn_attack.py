...
train_df = pd.read_csv('sha_ConfidenceScores.csv')
x_train = train_df.iloc[:, range(8)].values
x_train = np.sort(x_train, axis=1)
y_train = train_df.loc[:, ['label']].values
test_df = pd.read_csv('ori_ConfidenceScores.csv')
x_test = test_df.iloc[:, range(8)].values
x_test = np.sort(x_test, axis=1)
y_test = test_df.loc[:, ['label']].values

def attackerModel(input_dim=8):
    model = Sequential()
...
def evaluate(x_test, y_test):
    model = load_model('weights/nn_attack.hdf5',
