...
def preproc_im(im_path):
    try:
        image = cv2.imread(str(im_path), -1)
        processed = _resize_and_center_fundus(image, diameter=256)
        cv2.imwrite(str(OUT_DIR / str(im_path.stem + '.png')), processed)

        if processed is None:
            raise Exception
    except:
        # print(f"Preprocessing failed for image: {im_path.name}")
        return 1
    return 0


# ##### PROGRAM STARTS HERE #####
# rerun on test as well, and then combine later
IN_DIR = Path("data/test_data")
OUT_DIR = Path("data/preocessed_test")
OUT_DIR.mkdir(exist_ok=True, parents=True)

im_paths = list(IN_DIR.glob('*.jpeg'))

if __name__ == "__main__":
    with multiprocessing.Pool(multiprocessing.cpu_count()) as p:
        # map_output is a logical array (0=pass, 1=failed)
        map_output = list(tqdm(p.imap(preproc_im, im_paths), total=len(im_paths)))

    failed_im_paths = np.asarray(im_paths, dtype=object)[np.where(map_output)]
    for im_path in failed_im_paths:
        print(f"Preprocessing failed for image: {im_path.name}")
