...
model_path = 'weights/full_synth_weights.hdf5'
model_defense_path = 'weights/defenseFullSynth.hdf5'
train_img_path = 'data/preprocessed_train/'
test_img_path = 'data/preprocessed_test/'
shadow_img_path = 'data/preprocessed_shadow_train/'
train_df = pd.read_csv('data/fullSynthRealConfidenceScore.csv', index_col=0)
test_df = pd.read_csv('data/fullSynthRealConfidenceScore.csv', index_col=0)
shadow_df = pd.read_csv('data/fullSynthConfidenceScore.csv', index_col=0)
whole_df = pd.concat([train_df, test_df, shadow_df], axis=0, ignore_index=True)
print(whole_df)
...
def mask_model(img_path, dataframe, model_path, model_defense_path):
    ...
    dataFrame, _, _ = set_data(img_path, dataframe)
    user_label_dim = 2
    ...
    # dataFrame = np.c_[model.predict(dataFrame), label]
        print(success_fraction)
    return result_array

train = mask_model(train_img_path, train_df, model_path, model_defense_path)
test = mask_model(test_img_path, test_df, model_path, model_defense_path)
shadow = mask_model(shadow_img_path, shadow_df, model_path, model_defense_path)

def save_CSV(train, test, shadow):
    train = pd.DataFrame(train)
    test = pd.DataFrame(test)
    data = pd.concat([train, test, shadow], axis=0, ignore_index=True)
    data = pd.concat([data, whole_df], axis=1)
    data.to_csv('data/fullSynthNoiseData.csv', index=False)
    return data


save_CSV(train_df, test_df, shadow_df)