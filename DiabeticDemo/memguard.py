...
from defense_model import model_defense_optimize
from defense_model import precision, f1, recall
...
train_img_path = '/home/bo/Project/data/first_train/'
test_img_path = '/home/bo/Project/data/first_test/'
label_df = pd.read_csv('/home/bo/Project/data/first_label.csv', error_bad_lines=False, index_col=0)
...
def predict_model(img_path):
    ...
    print(model.layers[-2].output[0, :])
    layer_model = Model(inputs=model.input, outputs=model.layers[-2].output)
    dataFrame, label = set_data(img_path)
    print(layer_model.predict(dataFrame))
    print(layer_model.predict(dataFrame).shape)
...
def mask_model(img_path):
    ...
    npzdata = np.load('../MemguardAttacker/defense.npz', allow_pickle=True)
    weights = npzdata['x']
...
def save_CSV(train, test):
    ...
    data = shuffle(data)
    data.to_csv('../MemguardData/test_masks_all.csv', index=None)
    print(data)
    return data
